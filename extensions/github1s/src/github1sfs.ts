...
const insertGitHubRESTEntryToDirectory = (
    ...
    pathParts.forEach((part) => {
        if (
            !(
                current.entries || (current.entries = new Map<string, Entry>())
            ).has(part)
        ) {
            current.entries.set(
                part,
                createEntry('tree', current.uri, current.name)
            );
        }
...
export class GitHub1sFS
            ...
            if (entry instanceof Directory) {
                if (entry.entries === null) {
                    await this.readDirectory(
                        Uri.joinPath(entry.uri, entry.name)
                    );
                }
    ...
    getFuse = reuseable(
        async (authority): Promise<Fuse<GithubRESTEntry>> => {
            ...
            return getGithubAllFiles(owner, repo, ref).then(
                async (treeData) => {
