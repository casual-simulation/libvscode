...
// Added registerMessagePort to support registering custom message ports for external communication.
(<any>self).registerMessagePort = (name: string, port: MessagePort) => {
    nativePostMessage({
        type: '_registerMessagePort',
        name: name,
        port: port
    }, [port]);
};
