...
export class WebWorkerExtensionHost extends Disposable implements IExtensionHost {
    ...
    private async _startInsideIframe(webWorkerExtensionHostIframeSrc: string): Promise<IMessagePassingProtocol> {
        ...
        this._register(dom.addDisposableListener(window, 'message', (event) => {
            ...
            // begin libvscode change
            // add support for allowing custom message ports between
            // extensions and the outside world.
            if (data?.type === '_registerMessagePort') {
                console.log('Got custom message port!');
                (<any>globalThis)?.vscode?.registerMessagePort(data.name, data.port);
                return;
            }
            // end libvscode change
