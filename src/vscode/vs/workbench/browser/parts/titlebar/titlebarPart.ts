...
export class TitlebarPart extends Part implements ITitleService {
    ...
    get minimumHeight(): number {
        return (
            30 /
            (this.currentMenubarVisibility === 'hidden' ? getZoomFactor() : 1)
        );
    }
    ...
    private doUpdateTitle(): void {
        ...
        // libvscode Change - Remove changing window.document.title
    ...
    protected adjustTitleMarginToCenter(): void {
        if (this.customMenubar && this.menubar) {
            ...
            if (
                leftMarker >
                    (this.element.clientWidth - this.title.clientWidth) / 2 ||
                rightMarker <
                    (this.element.clientWidth + this.title.clientWidth) / 2
            ) {
...
registerThemingParticipant(
    (theme: IColorTheme, collector: ICssStyleCollector) => {
        ...
        const titlebarInactiveFg = theme.getColor(
            TITLE_BAR_INACTIVE_FOREGROUND
        );
