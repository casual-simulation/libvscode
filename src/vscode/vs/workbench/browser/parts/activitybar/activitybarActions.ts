            ...
            if (sessionInfo.sessions) {
                Object.keys(sessionInfo.sessions).forEach((accountName) => {
                    ...
                    const hasEmbedderAccountSession = sessionInfo.sessions[
                        ...
                        (session) => session.id === authenticationSession?.id
                    );
                    if (
                        !hasEmbedderAccountSession ||
                        authenticationSession?.canSignOut
                    ) {
...
registerThemingParticipant(
    (theme: IColorTheme, collector: ICssStyleCollector) => {
        const activityBarBackgroundColor = theme.getColor(
            ACTIVITY_BAR_BACKGROUND
        );
        ...
        const activityBarForegroundColor = theme.getColor(
            ACTIVITY_BAR_FOREGROUND
        );
