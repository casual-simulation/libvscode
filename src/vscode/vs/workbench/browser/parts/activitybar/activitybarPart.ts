...
export class ActivitybarPart extends Part implements IActivityBarService {
                    ...
                    getOnCompositeClickAction: (compositeId: string) =>
                        new Action(compositeId, '', '', true, () =>
                            this.viewsService.isViewContainerVisible(
                                ...
                                : this.viewsService.openViewContainer(
                                      compositeId
                                  )
                        ),
                    getContextMenuActions: () => {
                        ...
                        actions.push(
                            this.instantiationService.createInstance(
                                ...
                                ToggleSidebarPositionAction.getLabel(
                                    this.layoutService
                                )
                            )
                        );
                            ...
                            new Action(
                                ...
                                async () => {
                                    this.instantiationService.invokeFunction(
                                        (accessor) =>
                                            new ToggleActivityBarVisibilityAction().run(
                                                accessor
                                            )
                                    );
                                }
    ...
    private onDidChangeViewContainers(
        ...
        removed
            .filter(
                ({ location }) => location === ViewContainerLocation.Sidebar
            )
            .forEach(({ container }) =>
    ...
    private onDidChangeHomeBarVisibility(): void {
        if (this.homeBarContainer) {
            this.homeBarContainer.style.display = this
                .homeBarVisibilityPreference
                ? ''
                : 'none';
        }
    ...
    private showGlobalActivity(
        ...
        return toDisposable(() =>
            this.removeGlobalActivity(activityId, activity)
        );
    }
    ...
    private toActivity(
        ...
        return ActivitybarPart.toActivity(
            id,
            name,
            icon,
            focusCommand?.id || id
    ...
    private removeNotExistingComposites(): void {
        ...
        for (const { id } of this.cachedViewContainers) {
            if (
                viewContainers.every((viewContainer) => viewContainer.id !== id)
            ) {
                if (
    ...
    getVisibleViewContainerIds(): string[] {
        return this.compositeBar
            ...
            .filter(
                (v) =>
                    this.viewsService.getVisibleViewContainer(this.location)
                        ?.id === v.id || this.compositeBar.isPinned(v.id)
            )
            .map((v) => v.id);
    }
    ...
    private onDidStorageValueChange(e: IStorageValueChangeEvent): void {
        if (
            ...
            for (let index = 0; index < compositeItems.length; index++) {
                ...
                if (
                    !newCompositeItems.some(
                        ({ id }) => id === compositeItems[index].id
                    )
                ) {
