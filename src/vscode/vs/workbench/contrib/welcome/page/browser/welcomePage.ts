...
function isWelcomePageEnabled(
    ...
    if (!startupEditor.userValue && !startupEditor.workspaceValue) {
        const welcomeEnabled = configurationService.inspect(
            oldConfigurationKey
        );
        if (
    ...
    private onReady(
        ...
        recentlyOpened
            .then(({ workspaces }) => {
                ...
                const updateEntries = () => {
                    const listEntries = this.createListEntries(
                        workspacesToShow
                    );
                    while (ul.firstChild) {
            ...
            .then((extensions) => {
                ...
                */
                    ...
                    this.notificationService.info(
                        strings.alreadyInstalled.replace(
                            '{0}',
                ...
                const foundAndInstalled = installedExtension
                    ...
                    : this.extensionGalleryService
                          ...
                          .then(
                              (
                                  result
                              ): null | Promise<ILocalExtension | null> => {
                ...
                this.notificationService.prompt(
                    Severity.Info,
                    strings.reloadAfterInstall.replace(
                        '{0}',
                        extensionSuggestion.name
                    ),
                    [
                                ...
                                */
                                              this.telemetryService.publicLog(
                                                  strings.installedEvent,
                                                  {
                                                      ...
                                                      extensionId:
                                                          extensionSuggestion.id,
                                                      outcome: isPromiseCanceledError(
                                                          err
                                                      )
                                                          ? 'canceled'
                                                          : 'error',
