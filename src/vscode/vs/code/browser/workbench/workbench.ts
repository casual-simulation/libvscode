...
class PollingURLCallbackProvider
    ...
    create(options?: Partial<UriComponents>): URI {
        ...
        if (scheme) {
            queryValues.set(
                PollingURLCallbackProvider.QUERY_KEYS.SCHEME,
                scheme
            );
        }
        ...
        if (fragment) {
            queryValues.set(
                PollingURLCallbackProvider.QUERY_KEYS.FRAGMENT,
                fragment
            );
        }
        ...
        if (content.byteLength > 0) {
            try {
                this._onCallback.fire(
                    URI.revive(JSON.parse(content.toString()))
                );
            } catch (error) {
                console.error(error);
            }
...
class WorkspaceProvider implements IWorkspaceProvider {
    ...
    private createTargetUrl(
        ...
        // Folder
            ...
            targetHref = `${document.location.origin}${
                document.location.pathname
            }?${WorkspaceProvider.QUERY_PARAM_FOLDER}=${encodeURIComponent(
        ...
        // Workspace
            ...
            targetHref = `${document.location.origin}${
                document.location.pathname
            }?${WorkspaceProvider.QUERY_PARAM_WORKSPACE}=${encodeURIComponent(
